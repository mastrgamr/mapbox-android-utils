...
/**
 * Renders GeoJsonFeature objects onto the MapboxMap as Marker, Polyline and Polygon objects. Also
...
/* package */ class GeoJsonRenderer implements Observer {
     ...
     */
    /* package */ GeoJsonRenderer(MapboxMap map, HashMap<GeoJsonFeature, Object> features) {
        mMap = map;
        mFeatures = features;
        mLayerOnMap = false;
    ...
    /**
     * Gets the MapboxMap that GeoJsonFeature objects are being placed on
     *
     * @return MapboxMap
     */
    /* package */ MapboxMap getMap() {
        return mMap;
    }
     ...
     *
     * @param map MapboxMap to place GeoJsonFeature objects on
     */
    /* package */ void setMap(MapboxMap map) {
        for (GeoJsonFeature feature : getFeatures()) {
    ...
    private Polygon addPolygonToMap(GeoJsonPolygonStyle polygonStyle, GeoJsonPolygon polygon) {
        ...
        for (int i = POLYGON_INNER_COORDINATE_INDEX; i < polygon.getCoordinates().size();
                i++) {
            //FIXME(the fuh?): polygonOptions.addHole(polygon.getCoordinates().get(i));
        }
    ...
    private void redrawFeatureToMap(GeoJsonFeature feature, MapboxMap map) {
        removeFromMap(mFeatures.get(feature));
